...
Describe "ConvertFrom-ExcelToSQLInsert" {
    ...
    Context "When escaping single quotes" {
        ...
        It "Should escape with double single quotes".PadRight(90) {
            $expected = "INSERT INTO Sheet1 ('FirstName', 'LastName', 'Age') Values('John', 'D''Angelo', '');"
            $actual = ConvertFrom-ExcelToSQLInsert -Path $xlFile Sheet1 -SingleQuoteStyle "''"
            $actual | Should -Be $expected
        }
    
        It "Should escape with backslashes".PadRight(90) {
            $expected = "INSERT INTO Sheet1 ('FirstName', 'LastName', 'Age') Values('John', 'D\'Angelo', '');"
            $actual = ConvertFrom-ExcelToSQLInsert -Path $xlFile Sheet1 -SingleQuoteStyle "\'"
            $actual | Should -Be $expected
        }    
